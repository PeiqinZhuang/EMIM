...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., attn_type=None):
        super().__init__()
        ...
        if self.attn_type == 'extra_attn35':
            self.max_disp = 7
    ...
    def forward(self, x):
        if self.attn_type is None:
            ...
            q = rearrange(q, 'b n (h w) d -> b n h w d', h=H, w=W)
            q = q * self.scale

            k = rearrange(k, 'b n (h w) d -> (b n) d h w', h=H, w=W)
            k = self.padding(k)
            k = rearrange(k, '(b n) d h w -> b n h w d', b=B, n=self.num_heads)

            v = rearrange(v, 'b n (h w) d -> (b n) d h w', h=H, w=W)
            v = self.padding(v)
            v = rearrange(v, '(b n) d h w -> b n h w d', b=B, n=self.num_heads)

            attn = NATTENQKRPBFunction_With_RPB.apply(q, k, self.rpb)
            attn = F.softmax(attn, dim=-1)
            attn = self.attn_drop(attn)

            x =  NATTENAVFunction_With_RPB.apply(attn, v)
            x = rearrange(x, 'b n h w d -> b (h w) (n d)', b=B)
...
class SABlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, attn_type=None):
        super().__init__()
        self.pos_embed = nn.Conv2d(dim, dim, 3, padding=1, groups=dim)
        ...
        self.attn = Attention(
            ...
            attn_drop=attn_drop, proj_drop=drop, attn_type=attn_type)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
