...
#define DIMTHREADS 8
#define PIXELTHREADS 4

// Backward A threads
#define BATCHTHREADS_BACKWARD_A 16
#define DIMTHREADS_BACKWARD_A 4
#define PIXELTHREADS_BACKWARD_A 8

// Backward V threads
#define BATCHTHREADS_BACKWARD_V 32
#define DIMTHREADS_BACKWARD_V 8
#define PIXELTHREADS_BACKWARD_V 4

// Unrolls
#define BATCHUNROLL 2

#define DIMUNROLL_FORWARD 4
#define DIM_DIV_DIMUNROLL_FORWARD 8

#define DIMUNROLL_BACKWARD_A 8
#define DIM_DIV_DIMUNROLL_BACKWARD_A 4

#define DIMUNROLL_BACKWARD_V 2
#define DIM_DIV_DIMUNROLL_BACKWARD_V 16
...
std::vector<torch::Tensor> nattenav_cuda_forward(
    ...
    int64_t height = attn.size(2);
    int64_t width = attn.size(3);
...
std::vector<torch::Tensor> nattenav_cuda_backward(
    ...
    int64_t dim = value.size(4);
    int64_t KERNEL_SIZE_SQ = attn.size(4);
    int64_t value_height = value.size(2);
    int64_t value_width = value.size(3);
    ...
    auto d_value = torch::zeros(
            {batch_size, heads, value_height, value_width, dim}, value.options());
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(at::kHalf, at::kBFloat16, d_attn.scalar_type(), "nattena_backward_cuda", ([&] {
        ...
        if (KERNEL_SIZE_SQ == 49)
            nattenv_cuda_backward_kernel<7, 3, scalar_t><<<blocksv, threadsv, 0, stream>>>(d_out_a, d_value_a, attn_a, height, width,
                    batch_size, heads, dim, d_value.numel());
        else if (KERNEL_SIZE_SQ == 25)
