...
*/
    if (i < height && j < width && h < heads && ki < KERNEL_SIZE && kj < KERNEL_SIZE)
        for (int bo=0; bo < BATCHUNROLL && b + bo < batch_size; ++bo){
            ...
            for (int d=0; d < dim; ++d)
                updt += query[b+bo][h][i][j][d] * key[b+bo][h][i+ki][j+kj][d];
            attn[b+bo][h][i][j][ki*KERNEL_SIZE+kj] = updt + rpb[h][ki][kj];
        }
}
...
*/
    if (i < height && j < width && h < heads && ki < KERNEL_SIZE && kj < KERNEL_SIZE) {
        ...
        const int index = h * d_rpb.stride(0) + ki * d_rpb.stride(1) + kj * d_rpb.stride(2);
...
std::vector<torch::Tensor> nattenqkrpb_cuda_forward(
    ...
    int64_t KERNEL_SIZE_SQ = pow(RPB_MAX, 2);
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(at::kHalf, at::kBFloat16, query.scalar_type(), "nattenqk_forward_cuda", ([&] {
        ...
        if (KERNEL_SIZE_SQ == 49)
            nattenqkrpb_cuda_forward_kernel<7, 49, 3, 6, scalar_t, accscalar_t><<<blocks, threads, 0, stream>>>(query_a, key_a, rpb_a, attn_a, height, width, batch_size, heads, dim);
        else if (KERNEL_SIZE_SQ == 25)
            nattenqkrpb_cuda_forward_kernel<5, 25, 2, 4, scalar_t, accscalar_t><<<blocks, threads, 0, stream>>>(query_a, key_a, rpb_a, attn_a, height, width, batch_size, heads, dim);
        else if (KERNEL_SIZE_SQ == 9)
            nattenqkrpb_cuda_forward_kernel<3, 9, 1, 2, scalar_t, accscalar_t><<<blocks, threads, 0, stream>>>(query_a, key_a, rpb_a, attn_a, height, width, batch_size, heads, dim);
        else if (KERNEL_SIZE_SQ == 81)
            nattenqkrpb_cuda_forward_kernel<9, 81, 4, 8, scalar_t, accscalar_t><<<blocks, threads, 0, stream>>>(query_a, key_a, rpb_a, attn_a, height, width, batch_size, heads, dim);
        else if (KERNEL_SIZE_SQ == 121)
            nattenqkrpb_cuda_forward_kernel<11, 121, 5, 10, scalar_t, accscalar_t><<<blocks, threads, 0, stream>>>(query_a, key_a, rpb_a, attn_a, height, width, batch_size, heads, dim);
    }));
    return {attn};
}
...
std::vector<torch::Tensor> nattenqkrpb_cuda_backward(
    ...
    int64_t RPB_MAX = sqrt(KERNEL_SIZE_SQ);
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(at::kHalf, at::kBFloat16, d_query.scalar_type(), "nattenq_backward_cuda", ([&] {
        ...
        const auto key_a = key.packed_accessor32<scalar_t,5,torch::DefaultPtrTraits>();
        if (KERNEL_SIZE_SQ == 49)
            nattenq_cuda_backward_kernel<7, 3, scalar_t, accscalar_t><<<blocksq, threadsq, 0, stream>>>(d_query_a, d_attn_a, key_a, height, width, batch_size, heads, dim);
        else if (KERNEL_SIZE_SQ == 25)
            nattenq_cuda_backward_kernel<5, 2, scalar_t, accscalar_t><<<blocksq, threadsq, 0, stream>>>(d_query_a, d_attn_a, key_a, height, width, batch_size, heads, dim);
        else if (KERNEL_SIZE_SQ == 9)
            nattenq_cuda_backward_kernel<3, 1, scalar_t, accscalar_t><<<blocksq, threadsq, 0, stream>>>(d_query_a, d_attn_a, key_a, height, width, batch_size, heads, dim);
        else if (KERNEL_SIZE_SQ == 81)
            nattenq_cuda_backward_kernel<9, 4, scalar_t, accscalar_t><<<blocksq, threadsq, 0, stream>>>(d_query_a, d_attn_a, key_a, height, width, batch_size, heads, dim);
        else if (KERNEL_SIZE_SQ == 121)
            nattenq_cuda_backward_kernel<11, 5, scalar_t, accscalar_t><<<blocksq, threadsq, 0, stream>>>(d_query_a, d_attn_a, key_a, height, width, batch_size, heads, dim);
    }));
    AT_DISPATCH_FLOATING_TYPES_AND2(at::kHalf, at::kBFloat16, d_key.scalar_type(), "nattenk_backward_cuda", ([&] {
        ...
        const auto query_a = query.packed_accessor32<scalar_t,5,torch::DefaultPtrTraits>();
        if (KERNEL_SIZE_SQ == 49)
            nattenk_cuda_backward_kernel<7, 3, scalar_t><<<blocksk, threadsk, 0, stream>>>(d_key_a, d_attn_a, query_a, height, width, batch_size, heads, dim, d_key.numel());
        else if (KERNEL_SIZE_SQ == 25)
            nattenk_cuda_backward_kernel<5, 2, scalar_t><<<blocksk, threadsk, 0, stream>>>(d_key_a, d_attn_a, query_a, height, width, batch_size, heads, dim, d_key.numel());
        else if (KERNEL_SIZE_SQ == 9)
            nattenk_cuda_backward_kernel<3, 1, scalar_t><<<blocksk, threadsk, 0, stream>>>(d_key_a, d_attn_a, query_a, height, width, batch_size, heads, dim, d_key.numel());
        else if (KERNEL_SIZE_SQ == 81)
            nattenk_cuda_backward_kernel<9, 4, scalar_t><<<blocksk, threadsk, 0, stream>>>(d_key_a, d_attn_a, query_a, height, width, batch_size, heads, dim, d_key.numel());
        else if (KERNEL_SIZE_SQ == 121)
            nattenk_cuda_backward_kernel<11, 5, scalar_t><<<blocksk, threadsk, 0, stream>>>(d_key_a, d_attn_a, query_a, height, width, batch_size, heads, dim, d_key.numel());
    }));
