...
model_path = {
   ...
   'uniformer_small_k400': os.path.join(model_path, 'uniformer_small_k400.pth'),
   'uniformer_base_in1k': os.path.join(model_path, 'uniformer_base_in1k.pth'),
   'uniformer_base_k400': os.path.join(model_path, 'uniformer_base_k400.pth'),
}
...
class SABlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 attn_type=None, temporal_attn='',  max_disp=7, layer_scale=False):
        super().__init__()
        self.pos_embed = conv_3x3x3(dim, dim, groups=dim)
        ...
        self.attn_type = attn_type
        print('SABlock Attention Type:', self.attn_type, 'Layer_scale:', layer_scale,  flush=True)
...
class Uniformer(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        uniformer_attn_type = cfg.UNIFORMER.ATTN_TYPE
        self.use_checkpoint = cfg.MODEL.USE_CHECKPOINT
        self.checkpoint_num = cfg.MODEL.CHECKPOINT_NUM
        attn_type = cfg.UNIFORMER.ATTN_TYPE

        attn_type = cfg.UNIFORMER.ATTN_TYPE
        attn_drop_path = cfg.UNIFORMER.ATTN_DROP_PATH
        temporal_attn = cfg.UNIFORMER.TEMPORAL_ATTN
        max_disp = cfg.UNIFORMER.ATTN_MAX_DISP
        consensus = cfg.UNIFORMER.CONSENSUS
        continuous_attn = cfg.UNIFORMER.CONTINUOUS_ATTN
        layer_scale = cfg.UNIFORMER.LAYER_SCALE
                    ...
                    if 'lg' == continuous_attn:
                        if i % 2 == 0:
                            blocks3.append(SplitSABlock(
                                ...
                                attn_type=attn_type[0], temporal_attn=temporal_attn, max_disp=max_disp[0], layer_scale=layer_scale))
                        else:
                            blocks3.append(SplitSABlock(
                                dim=embed_dim[2], num_heads=num_heads[2], mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i+depth[0]+depth[1]], norm_layer=norm_layer, img_size=img_size // 16,
                                attn_type=None, temporal_attn=temporal_attn,  max_disp=max_disp[0], layer_scale=layer_scale))
                    ...
                    if 'lg' == continuous_attn:
                        if i % 2 == 0:
                            blocks4.append(SplitSABlock(
                                ...
                                attn_type=attn_type[1], temporal_attn=temporal_attn,  max_disp=max_disp[1], layer_scale=layer_scale))
                        else:
                            blocks4.append(SplitSABlock(
                                dim=embed_dim[3], num_heads=num_heads[3], mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i+depth[0]+depth[1]+depth[2]], norm_layer=norm_layer, img_size=img_size // 32,
                                attn_type=None, temporal_attn=temporal_attn, max_disp=max_disp[1], layer_scale=layer_scale))
