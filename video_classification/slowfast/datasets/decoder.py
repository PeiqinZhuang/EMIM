...
def temporal_sampling(video_length, start_idx, end_idx, num_samples):
    """
    ...
    # index = torch.clamp(index, 0, video_length - 1).long()
    # frames = torch.index_select(frames, 0, index)
    index = np.linspace(start_idx, end_idx, num_samples)
    index = np.clip(index, 0, video_length -1).astype(np.int)


    return index


def temporal_sampling_kinetics(frames, start_idx, end_idx, num_samples):
    """
    Given the start and end frame index, sample num_samples frames between
    the start and end with equal interval.
    Args:
...
def decode(
    ...
    if backend in ["pyav", "torchvision"]:
        ...
        frames = temporal_sampling_kinetics(frames, start_idx, end_idx, num_frames)
    elif backend == "decord":
...
def decode_image(
    ...
    # Perform temporal sampling from the decoded video.
    index = temporal_sampling(video_length, start_idx, end_idx, num_frames)

    frames = []
    for idx in index:
        frame_path = os.path.join(video_path, 'img_{:05d}.jpg'.format(idx+1))
        try:
            img_bytes = file_client.get(frame_path)
        except:
            print('Video Length:', video_length)
            print('Failed:', frame_path, flush=True)
